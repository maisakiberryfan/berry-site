...
//
// ── 為什麼從 DuckDB-WASM 換掉 ────────────────────────────────────────────
// 舊版首次執行要下載 ~20MB 引擎（jsDelivr CDN）＋ 5MB parquet 快照，實測要等好幾秒。
// sql.js 的 wasm 只有 ~0.64MB（gzip 後更小）且 self-host，資料則直接取自瀏覽器裡
// 已有的 store 快取（dataset.js）——不再有 sqldata.m-b.win 這條外部管線，
// 資料也從「每日快照」變成「與表格頁同一份、背景校正過的最新值」。
//
// ── 載入策略 ────────────────────────────────────────────────────────────
// 1. 本模組被 QueryPanel.svelte 靜態 import，QueryPanel 又被 Analytics.svelte
//    動態 import ⇒ 主 bundle 增量為 0。
// 2. sql.js 本體在 boot() 內二次動態 import，因此預載查詢面板 UI 時
//    不會連帶下載引擎——直到使用者第一次按下「執行查詢」為止。
// 3. .wasm 走 vite 的 ?url self-host（進 dist/assets），不打 CDN；
...
/** 由記憶體中的寬表列（重）建 berry_data；16k 列包在單一 transaction 內 */
 ...
 * columns: [{ key, type, numeric, time }]，rows 為純物件（可直接餵 CSV 匯出）。
 *
