// 查詢資料集：把瀏覽器裡「已經有」的快取（setlist ＋ streamlist ＋ songlist）
// 攤平成一張寬表，給統計面板的 JS 聚合與查詢面板的 sql.js 建表共用。
//
// ── 為什麼不再抓 parquet ────────────────────────────────────────────────
// 舊版走 DuckDB-WASM + https://sqldata.m-b.win/berry-data.parquet：引擎約 20MB、
// parquet 是每日快照。但同一份資料本來就躺在 IndexedDB（tool.js 的表格快取），
...
/** songID → songlist row（genre / tieup / songNote 的正本） */
export function indexSongs(songRows) {
  const map = new Map()
  for (const s of songRows || []) {
    if (s?.songID != null) map.set(s.songID, s)
  }
  return map
}

/** streamID → streamlist row（streamTitle / categories 的來源） */
export function indexStreams(streamRows) {
  const map = new Map()
  for (const s of streamRows || []) {
    if (s?.streamID) map.set(s.streamID, s)
  }
  return map
}
...
let memoStreams = null
let memoSongs = null
let memoResult = null

/**
 * @param {Array} setlistRows  setlist VIEW 的列（含 time／songID／startTime 等）
 * @param {Map}   streamMap    indexStreams() 的結果（補 streamTitle / categories）
 * @param {Map}   songMap      indexSongs() 的結果（補 genre / tieup / songNote）
 * @returns {Array<object>}    寬表列（欄位同 DATASET_COLUMNS）
 */
export function buildDataset(setlistRows, streamMap, songMap) {
  if (memoResult && memoSetlist === setlistRows && memoStreams === streamMap && memoSongs === songMap) {
    return memoResult
  }
  ...
  for (let i = 0; i < setlistRows.length; i++) {
    ...
    const stream = r.streamID ? streamMap?.get(r.streamID) : null
    const localTime = toLocalWallClock(r.time)
    const start = num(r.startTime)
    const end = num(r.endTime)
    rows[i] = {
      streamID: text(r.streamID),
      streamTitle: text(stream?.title ?? null),
      time: localTime,
      categories: categoriesText(stream?.categories),
